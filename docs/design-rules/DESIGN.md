...
- **Session-loading wordmark sheen(会话加载字标扫光)** — the sixth sanctioned motion class (approved 2026-08-10), **only** for Desktop `BrandLoadingMark` while `MessageStream` defers its message tree during a session switch:
  - **Definition**: the active theme's official Light/Dark Lex wordmark sits centered in the empty message viewport at 35% opacity; after a 200ms delayed-reveal threshold, one narrow neutral highlight traverses the wordmark shape. The asset alpha channel is the mask, so motion is confined to the glyphs and never paints a panel/background. This is functional loading feedback for a measured long switch, not a decorative brand loop: ordinary sessions complete before the reveal threshold and never show it; the mark unmounts immediately when the message tree mounts.
...
The new-session brand block is fixed at a 50×50px square icon + 110×37.5px horizontal logo with a 9px gap (2026-07 design). `ThemeBrandLockup` is the sole renderer. The built-in Light/Dark themes keep their stable IDs but use the Lex assistant avatar (`assets/branding/lex-assistant-avatar.png`) plus the matching Lex SVG wordmark (`assets/login/lex-wordmark.svg` / `lex-wordmark-dark.svg`). Never rescale the finalized lockup; local theme images are cropped at load time to their alpha visible bounds (source files untouched).
...
> Brand artwork colors are intrinsic to the exported Lex assets and sit outside the UI token system. Do not recolor, filter, or reconstruct the wordmark or assistant in component CSS.

Lex brand bitmap sources live under `assets/branding/`: master 1868×1868, login hero 934×934, avatar 200×200, and share crop 1024×1024; all are transparent RGBA PNGs derived from the same approved character. The old `assets/splash/*` and `assets/login/wordmark*.png` files are compatibility/source remnants and are not valid new Lex surface inputs.
...
- **Lockup**: Desktop Lex exports use `lex-assistant-share.png` as a static 40×40px crop (8px radius), followed by the active theme's 24px-high Lex wordmark with an 8px gap. Current Lex does not ship a Mobile client; upstream Cindy Mobile keeps its existing Cindy brand contract: a product-approved character crop at 22×22px (6px radius), followed by an 18px-high wordmark with a 6px gap. Do not append a website or regional host. Each character keeps its source asset's original color and opacity without component-authored filtering.
- **Constraints**: no animation, shadow, decorative background, additional brand color, enlarged hero treatment, or alternate character composition. This approval identifies the source of an exported conversation in each named product; it is not precedent for adding mascots to cards, dialogs, tool output, or other share-adjacent UI.
...
3. **`LoginBrandStage` 资产按模式切**：亮色 / 深色分别使用登录专用 Lex 字标 `assets/login/lex-wordmark.svg` / `lex-wordmark-dark.svg`；旧 Cindy 手写 slogan 不再渲染。立绘两模式共用 `assets/branding/lex-assistant-hero.png`，2x source 为 `lex-assistant-master.png`，不加组件侧滤镜或改色。
...
**主题跟随（产品逻辑）**：用户**首次**打开 Lex → **亮色**登录界面（默认）；**第二次起** → 登录界面跟随用户上一次使用的 **light / dark 模式**（登录页只认 light / dark，不随具体扩展主题，见 §16.2）。这需要持久化「上次登录模式」+ 首次默认逻辑，超出纯 token 补范围，是一段状态逻辑，在暗色实现 PR 内一并落地。
