...
capturing Lex process and accessibility in the actual input helper. It opens
...
The phone shows separate permission states and can request this Lex guide on
...
registered Lex application top-level frame. Account capability alone does not
...
applications and separate Lex processes are outside that guard.
...
Lex now has a macOS native compatibility capture child for a logged-in user.
...
End-to-end checks through two authenticated Lex clients on this machine passed:
...
Authorization records a live process handle for the Lex process explicitly
approved through the elevated setup helper. Executable path alone is insufficient:
plugin utility processes share Lex.exe. The service starts on demand and exits when its approved Lex process exits.
Restarting Lex or the service requires fresh administrator approval. This is a current limitation, not unattended
...
Windows policy decides whether software SAS is allowed; Lex never changes it.
