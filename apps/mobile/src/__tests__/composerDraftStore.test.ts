...
describe('composerDraftStore', () => {
  it.each(['both', 'text-only', 'new-draft'] as const)('reconciles a committed send after a crash with %s remaining', async (state) => {
    ...
    await expect(readComposerDraft('s1')).resolves.toBe(state === 'new-draft' ? 'later edit' : null);
    await expect(readComposerDocumentDraft('s1')).resolves.toEqual(
      state === 'new-draft' ? textComposerDocument('later edit') : state === 'text-only' ? null : after,
    );
  });
  ...
  it('propagates draft handoff storage failure and retries the unmodified disk proof', async () => {
    ...
    await expect(reconcileCommittedComposerDraft('s1', { before, after: emptyComposerDocument() }, () => {}))
