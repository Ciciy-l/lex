...
export function createDurableOutbox(storage: OutboxStorage, reconcileDraft?: (
  ...
  const mutate = async (
    ...
    return serialize(async () => {
      ...
      if (next) {
        records = adding
          ? [...records, next].sort((a, b) => a.createdAt - b.createdAt)
          : records.map((current) => keyFor(current) === keyFor(record) ? next : current);
      } else {
        records = records.filter((current) => keyFor(current) !== keyFor(record));
      }
      emit();
      return next;
    });
  ...
  const store = {
    ...
    async add(record: DurableOutboxRecord) {
      ...
      const previousInSession = records
        .filter((current) => current.accountId === record.accountId
          && current.deviceId === record.deviceId
          && current.item.sessionId === record.item.sessionId)
        .reduce((latest, current) => Math.max(latest, current.createdAt), Number.NEGATIVE_INFINITY);
      const orderedRecord = Number.isFinite(previousInSession) && record.createdAt <= previousInSession
        ? { ...record, createdAt: previousInSession + 1 }
        : record;
      await mutate(orderedRecord, orderedRecord, true);
    },
    update: (
      record: DurableOutboxRecord,
