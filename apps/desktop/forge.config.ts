...
const LEX_SOURCE_METADATA_PATH = path.join(__dirname, 'resources', 'lex-source.json');
 ...
 * Lex can identify the exact source checkout that produced it, even when
 * package.json still contains the 0.0.0 placeholder.
 */
function stageLexSourceMetadata(): void {
  const sourceCommit = resolveSourceCommit();
  const builtAt = formatLocalBuildTime();
  fs.writeFileSync(
    LEX_SOURCE_METADATA_PATH,
    `${JSON.stringify({ sourceCommit, builtAt }, null, 2)}\n`,
    'utf8',
  );
  console.log(`[forge:prePackage] staged Lex source metadata (${sourceCommit || 'unknown commit'})`);
}

function removeLexSourceMetadata(): void {
  fs.rmSync(LEX_SOURCE_METADATA_PATH, { force: true });
}
