...
/**
 * 计费面的币种由当前 Cindy 账号服务区决定（服务端下发的金额本身不带币种）。所有展示余额 /
 * 充值金额的界面必须用同一映射，否则同一笔钱会在两个界面显示成不同货币。
 */
export function billingCurrencyForRealm(realm: 'cn' | 'global'): 'cny' | 'usd' {
  return realm === 'global' ? 'usd' : 'cny';
}
