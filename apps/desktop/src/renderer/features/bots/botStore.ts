...
function vendorForHarness(harness: BotCapabilities['harness']): 'cc' | 'codex' | 'pi' | 'omp' {
...
function defaultBotModelSettings(vendor: ReturnType<typeof vendorForHarness>): BotModelOverride {
  ...
  // OMP has no product-owned model seed. Resolve it from the connected provider
  // catalog rather than falling back to Claude's catalog for an unconfigured Bot.
  const preferredModel = vendor === 'omp' ? '' : getDefaultModelForVendor(vendor).id;
  const picked = pickConnectedModelForAgent(providers, agent, preferredModel);
...
function readGlobalModelOverrides(): Partial<Record<BotModelVendor, BotModelOverride>> {
  ...
  try {
    ...
    for (const vendor of ['cc', 'codex', 'pi', 'omp'] as const) {
...
export function getEffectiveBotModelChain(
  ...
  return defaultBotModelChain({
    ...
    availableAgents: availableAgents ?? new Set(),
...
export async function addBotProfileAndWait(input: CreateBotProfileInput): Promise<BotProfile> {
  ...
  const needsDynamicDefault =
    (harness === 'pi' || harness === 'omp') && input.capabilities?.model === undefined;
  if (needsDynamicDefault && getCachedProvidersSnapshot() === null && typeof window !== 'undefined') {
