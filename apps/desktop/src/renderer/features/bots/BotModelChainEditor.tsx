...
function vendorFor(harness: BotHarness): 'cc' | 'codex' | 'pi' | 'omp' {
  return harness === 'claude' ? 'cc' : harness;
}

function harnessFor(vendor: 'cc' | 'codex' | 'pi' | 'omp'): BotHarness {
  return vendor === 'cc' ? 'claude' : vendor;
}

function agentKindFor(vendor: 'cc' | 'codex' | 'pi' | 'omp'): AgentKind {
  return vendor === 'cc' ? 'claude-code' : vendor;
}

function defaultRoute(vendor: 'cc' | 'codex' | 'pi' | 'omp'): BotModelRoute {
...
export function BotModelChainEditor({
  ...
  const visibleVendors = (['pi', 'codex', 'cc', 'omp'] as const)
  ...
  // ModelSelector can emit a wider UnifiedEngine set. Keep this explicit guard
  // so a Bot route is created only for a runtime visible to this editor.
