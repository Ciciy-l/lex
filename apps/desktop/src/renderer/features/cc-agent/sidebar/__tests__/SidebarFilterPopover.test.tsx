...
describe('sidebar display settings menu', () => {
  ...
  it('keeps status independent from content filters and exposes selectable Pi and OMP harnesses', async () => {
    ...
    expect(
      within(harness)
        .getAllByRole('menuitem')
        .map((row) => row.textContent),
    ).toEqual(['全部', 'Claude Code', 'Codex', 'Pi', 'OMP']);
    fireEvent.click(within(harness).getByRole('menuitem', { name: 'Pi' }));
    expect(filter.setVendor).toHaveBeenCalledWith('pi');
    fireEvent.click(within(harness).getByRole('menuitem', { name: 'OMP' }));
    expect(filter.setVendor).toHaveBeenCalledWith('omp');
