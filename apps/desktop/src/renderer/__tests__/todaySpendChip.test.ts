...
const usageCardModelPath = resolve(__dirname, '..', 'components', 'status', 'usageCardModel.ts');
...
const usageCardModelSource = readFileSync(usageCardModelPath, 'utf8').replace(/\r\n/g, '\n');
...
describe('TodaySpendChip dashboard routing', () => {
  it('keeps the latest user-round total separate while aggregating its token and model details', () => {
    expect(source).toContain('message.userTurnMoney?.amount');
    expect(source).toContain('const userTurnCostUsd =');
    ...
    expect(quotaHoverCardSource).toContain('turnUsage.isUserTurnTotal');
  });
  ...
  it('keeps CC/Pi ChatGPT and xAI bridge sessions on their subscription-specific paths', () => {
  ...
  it('shows the shared mobile Codex reset-credit summary for local Desktop sessions', () => {
    ...
    // 上游将详情渲染收敛到共享 usage-card model，Lex 的重置额度数据仍沿同一链路。
    expect(usageCardModelSource).toContain('resetSummary?.hasResetCreditCount');
    expect(usageCardModelSource).toContain('resetSummary?.earliestExpiryAt');
    expect(usageCardModelSource).toContain("t('todaySpend.codex.resetCreditsAvailableLine'");
    expect(usageCardModelSource).toContain("t('todaySpend.codex.resetCreditEarliestExpiryLine'");
    expect(usageCardModelSource).not.toContain('resetSummary?.resetRows');
  ...
  it('keeps one route-independent session total while provider-specific quota details follow the current model', () => {
    ...
    expect(source).toContain('const quotaCardSessionUsage = toQuotaHoverCardSessionUsage(sessionUsage, sessionTokens);');
    expect(source).toContain('function toQuotaHoverCardSessionUsage(');
    expect(source).toContain('sessionUsage={quotaCardSessionUsage}');
    expect(source).toContain('function getCodexChipWindows(');
    expect(quotaHoverCardSource).toContain("'todaySpend.codex.sessionValueLabel'");
    expect(quotaHoverCardSource).toContain("'todaySpend.tooltip.sessionUsed'");
    expect(source).toContain('function formatCompactTimeUntilReset(');
    expect(source).toContain('function toCodexChipWindow(');
    expect(source).toContain('buildCodexUsageCard(accountUsage, codexResetSummary, t, windowLabelNowMs');
    expect(usageCardModelSource).toContain('todaySpend.codex.balanceDepleted');
