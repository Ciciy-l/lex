...
export interface EnvCheckContextValue {
  ...
  /** D 场景顺序下载阶段：1 / 2 / 3 / 4；B/C 场景未定义。 */
  step?: 1 | 2 | 3 | 4;
  /** D 场景 = 本次需要下载的二进制段数(2、3 或 4);B/C 场景未定义。 */
  totalSteps?: 2 | 3 | 4;
  /**
...
export function EnvCheckProvider({ children }: { children: ReactNode }) {
  ...
  const [step, setStep] = useState<1 | 2 | 3 | 4 | undefined>(undefined);
  const [totalSteps, setTotalSteps] = useState<2 | 3 | 4 | undefined>(undefined);
