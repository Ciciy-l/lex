...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string) =>
      ({ 'sidebar.tabs.bots': '伙伴', 'bots.backToTasks': '返回任务' })[key] ?? key,
  }),
}));
...
// Teammates stays behind Lex's experimental opt-in. These navigation tests cover
// the enabled workspace rather than accidentally relying on the default-off state.
vi.mock('@/hooks/useExperimentalFeatures', () => ({
  useExperimentalFlag: () => ({ enabled: true, setEnabled: vi.fn() }),
}));
