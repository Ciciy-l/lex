...
beforeEach(async () => {
  ...
  auth.loadLoginState.mockImplementation(async () => {
    ...
    return loadEpoch === epoch
      ? { success: true, state: identifierState, realm: 'global' }
      : { success: false, code: 'AUTH_FLOW_SUPERSEDED', state: identifierState, realm: 'global' };
  });
  auth.beginAddAccount.mockImplementation(async () => {
    epoch += 1;
    await Promise.resolve();
    publishState(identifierState);
    return { success: true, state: identifierState, realm: 'global' };
  });
});
  ...
  it.each([false, true])(
    ...
    async (addAccount) => {
      ...
      initialize.mockImplementation(async () => {
        ...
        publishState(state);
        return { success: false, code: state.code, state, realm: 'global' };
      });
