...
function presetRuntimeBaseUrl(
  ...
  for (const [sourceAgent, endpoint] of Object.entries(edited)) {
    const source = presetRuntimeForAgent(preset, sourceAgent as AgentKind);
...
export function AddProviderWizard({
  ...
  // Native credentials alone do not mean this Lex account has added the local
  ...
  const startFetch = useCallback(async () => {
    ...
    const results = await Promise.all(
      agents.flatMap((agent) => {
        const originalRuntime = presetRuntimeForAgent(preset, agent);
    ...
    for (const result of results) {
      ...
      const runtime = presetRuntimeForAgent(preset, result.agent);
      const sibling = results.find(other => other.ok && !other.route && other.modelsUrl === result.modelsUrl
        ...
        && presetRuntimeForAgent(preset, other.agent)?.wireProtocol === runtime?.wireProtocol
        && JSON.stringify(presetRuntimeForAgent(preset, other.agent)?.headers ?? {}) === JSON.stringify(runtime?.headers ?? {}));
