import { providerEndpointBindings, BUNDLED_CATALOG, classifyModel, isChatEligible, mergeModelMetadata } from '@cindy/model-providers';
/**
 ...
 * omp 经 Cindy 的受管兼容代理接入，支持 Anthropic Messages、OpenAI Responses 和
 * Chat Completions 三种 wire protocol；Google 原生协议没有对应的 OMP 前门。
 */
...
function wireProtocolCopyKey(
  ...
  const suffix = protocol === 'anthropic-messages'
    ? 'Anthropic'
...
function defaultWireFor(agent: DialogAgentKind): ProviderWireProtocol {
  ...
  // OMP's managed proxy defaults to its Anthropic Messages front door. Other
  // supported wire formats are stored explicitly and resolved by omp-host.
...
export function ProviderConnectionDialog({
  ...
  // 新建和编辑态都加载预设模板；按当前账号服务区排序，不随 UI 语言变化。
  useEffect(() => {
    let cancelled = false;
          ...
          >
            {!templateBound && (
              <div className="flex flex-col gap-[7px]">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {customProviderWireProtocolsForAgent(activeTab).map((option) => (
                    <button
                      key={option.value}
