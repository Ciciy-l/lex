...
it('offers an OMP tab where the upstream protocol can be chosen', async () => {
  ...
  expect(
    screen.queryByRole('button', {
      name: 'settings.providers.custom.modelProtocol.google',
    }),
  ).toBeNull();
});
...
it('derives the OMP runtime from the preset claude-code runtime', async () => {
  ...
  fireEvent.click(screen.getByRole('tab', { name: 'settings.providers.custom.protocol.omp' }));
  // The current dialog summarizes preset models instead of rendering an input
  // for every model row. Verify the persisted configuration instead of relying
  // on the retired per-row editor.
  fireEvent.click(screen.getByRole('button', { name: 'settings.providers.custom.save' }));
  await waitFor(() => expect(customProviderMocks.createCustomProvider).toHaveBeenCalledOnce());
  expect(customProviderMocks.createCustomProvider.mock.calls[0][0].runtimes.omp).toMatchObject({
    baseUrl: 'https://api.example.test/anthropic',
