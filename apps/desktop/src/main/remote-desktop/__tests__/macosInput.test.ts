...
it('binds packaged native remote-desktop helpers to Lex product identities', async () => {
  ...
  const [macInput, windowsSecurity, windowsIdentity, windowsLib, windowsMain, windowsService, capture, installer] = await Promise.all([
    ...
    readFile(path.join(native, 'windows-host', 'src', 'lib.rs'), 'utf8'),
    readFile(path.join(native, 'windows-host', 'src', 'main.rs'), 'utf8'),
    readFile(path.join(native, 'windows-host', 'src', 'service.rs'), 'utf8'),
    readFile(path.join(native, 'macos-capture.m'), 'utf8'),
    readFile(path.resolve(import.meta.dirname, '../../../../resources/installer.nsh'), 'utf8'),
  ]);
  ...
  expect(windowsSecurity).toContain('assert!(!is_lex_main_executable(OsStr::new("Cindy.exe")))');
  expect(windowsIdentity).toContain('const SERVICE_NAME_PREFIX: &str = "LexRemoteDesktop"');
  expect(windowsIdentity).not.toContain('CindyRemoteDesktop');
  expect(windowsLib).toContain('identity::service_name_for_path(&path)');
  expect(windowsLib).not.toContain('CindyRemoteDesktop-');
  expect(windowsMain).toContain('identity::service_name_for_path(&path)');
  expect(windowsService).toContain('wide("Lex Remote Desktop")');
  expect(capture).toContain('"Lex remote desktop viewer"');
  expect(installer).toContain('Could not stop the ${PRODUCT_FILENAME} remote desktop service');
});
