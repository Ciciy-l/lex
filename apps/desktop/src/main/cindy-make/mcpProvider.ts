...
/**
 * Complete a Cindy Make task from a host-owned session context.  Claude's
 * shared bridges resolve that context at call time, while OMP closes over its
 * immutable per-process context; both routes deliberately use this same
 * marker and business-session check.
 */
async function reportComplete(
  deps: CindyMakeMcpDeps,
  context: Pick<McpProviderContext, 'sessionId' | 'vendorOptions'>,
) {
  const sessionId =
    typeof context.sessionId === 'string' && context.sessionId.trim()
      ? context.sessionId.trim()
      : undefined;
  if (!sessionId || !isCindyMakeVendorOptions(context.vendorOptions)) {
    return result(
      {
        ok: false,
    ...
    toClaudeSdkConfig: (ctx) => {
      ...
      server.tool(
        ...
        async () => {
          ...
          return reportComplete(deps, runtime);
        },
      );
      return { type: 'sdk', name: CINDY_MAKE_MCP_SERVER_NAME, instance: server };
    },
    // OMP cannot consume an in-process MCP server. Its per-session RPC host
    // tools are intentionally narrow: the marker controls visibility and the
    // closure supplies the host-owned session identity.
    toOmpRpcHostTools: (ctx) => {
      if (ctx.agentKind !== 'omp' || !isCindyMakeVendorOptions(ctx.vendorOptions)) {
        return null;
      }
      return [
        {
          name: CINDY_MAKE_REPORT_COMPLETE_TOOL,
          label: CINDY_MAKE_REPORT_COMPLETE_TOOL,
          description: CINDY_MAKE_REPORT_COMPLETE_DESCRIPTION,
          parameters: {},
          execute: async () => reportComplete(deps, ctx),
        },
      ];
    },
  };
