...
import { CINDY_PERSONAL_BRANCH, CINDY_SOURCE_REPOSITORY } from './sourcePaths.js';
import { checkMakeToolVersion, untilAborted } from './doctor.js';
export { CINDY_SOURCE_REPOSITORY, makeSourceRoot, makeSourceCheckoutPath } from './sourcePaths.js';
...
async function git(
  ...
  return runSourceGit(gitExecutable, env.processEnvironment(), args, cwd, signal, onProgress);
}
...
async function prepareCindySourceInternal(
  ...
  try {
    ...
    if (!gitExecutable)
      throw Object.assign(new Error('gitUnavailable'), { code: 'gitUnavailable' });
    const resolvedRef = await resolveRemoteRef(gitExecutable, env, target, root, signal);
    ...
    if (await exists(gitDir)) {
      ...
      const dirty = await git(gitExecutable, env, ['status', '--porcelain'], sourcePath, signal);
    ...
    if (!hasPersonal.trim()) {
      await git(
        gitExecutable,
        env,
        ['branch', CINDY_PERSONAL_BRANCH, baseCommit],
        sourcePath,
        signal,
      );
    }
    ...
    await git(gitExecutable, env, ['checkout', CINDY_PERSONAL_BRANCH], sourcePath, signal);
    const commit = await git(gitExecutable, env, ['rev-parse', 'HEAD'], sourcePath, signal);
