...
function markedOmp(sessionId = 'make-omp-1'): McpProviderContext {
  return {
    ...marked(sessionId),
    agentKind: 'omp',
  };
...
describe('cindy_make report_complete', () => {
  ...
  it('projects the same marked completion tool to OMP through the bounded RPC adapter', async () => {
    const reportCompletion = vi.fn(async () => undefined);
    const provider = createCindyMakeMcpProvider({ reportCompletion, logger });
    const tools = provider.toOmpRpcHostTools?.(markedOmp());
    ...
    expect(result.isError).toBeFalsy();
    expect(reportCompletion).toHaveBeenCalledWith('make-omp-1');
  });

  it('does not project cindy_make to an unmarked OMP session and fails closed without a session id', async () => {
    const reportCompletion = vi.fn(async () => undefined);
    const provider = createCindyMakeMcpProvider({ reportCompletion, logger });
    expect(
      provider.toOmpRpcHostTools?.({ agentKind: 'omp', workingDir: 'C:\repo', vendorOptions: {} }),
    ).toBeNull();
