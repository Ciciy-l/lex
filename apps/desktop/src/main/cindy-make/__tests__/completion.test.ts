...
import {
  ...
  commitVerifiedCindyMakeChanges,
...
describe('commitVerifiedCindyMakeChanges', () => {
  it('does not run add or commit when a later worktree verification fails', async () => {
    const verificationFailure = new Error('worktree replaced');
    let checks = 0;
    const verify = vi.fn(async () => {
      checks += 1;
      if (checks >= 4) throw verificationFailure;
      return { path: '/managed/worktree' };
    });
    const run = vi.fn(async (args: string[]) => {
      if (args[0] === 'rev-parse' && args[1] === '--abbrev-ref') return 'cindy-make/run-1';
      if (args[0] === 'status') return ' M task.ts\n';
      throw new Error(`unexpected ${args.join(' ')}`);
    });

    await expect(commitVerifiedCindyMakeChanges(verify, run, 'Cindy Make: task')).rejects.toBe(
      verificationFailure,
    );
    expect(run.mock.calls.map(([args]) => args[0])).toEqual(['rev-parse', 'status']);
  });

  it('keeps ordinary Git failures as sparse completion facts', async () => {
    const verify = vi.fn(async () => ({ path: '/managed/worktree' }));
    const run = vi.fn(async (args: string[]) => {
      if (args[0] === 'rev-parse' && args[1] === '--abbrev-ref') return 'cindy-make/run-1';
      if (args[0] === 'status') throw new Error('index locked');
      if (args[0] === 'rev-parse') return 'abcdef1234567';
      throw new Error(`unexpected ${args.join(' ')}`);
    });

    await expect(commitVerifiedCindyMakeChanges(verify, run, 'Cindy Make: task')).resolves.toEqual({
      branch: 'cindy-make/run-1',
      commit: 'abcdef1234567',
    });
