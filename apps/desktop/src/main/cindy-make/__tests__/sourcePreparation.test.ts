...
describe('Git-only source preparation', () => {
  ...
  it.each([
    ...
    async (platform, source, existing) => {
      ...
      vi.mocked(runSourceGit).mockImplementation(async (gitExecutable, processEnv, args) => {
        expect(gitExecutable).toBe(selectedGit);
      ...
      expect(createBranch?.[2]).toEqual(['branch', 'cindy-personal', '0123456789abcdef']);
      expect(createBranch?.[3]).toBe(sourcePath);
      const checkout = gitCalls.find(([, , args]) => args[0] === 'checkout');
      expect(checkout?.[2]).toEqual(['checkout', 'cindy-personal']);
      expect(checkout?.[3]).toBe(sourcePath);
      ...
      expect(gitCalls.every(([gitExecutable]) => gitExecutable === selectedGit)).toBe(true);
      const remoteRefLookup = gitCalls.find(([, , args]) => args[0] === 'ls-remote');
      expect(remoteRefLookup?.[3]).toBe(root);
  ...
  it('keeps an existing personal branch and never resets it to upstream', async () => {
    ...
    vi.mocked(runSourceGit).mockImplementation(async (_gitExecutable, _env, args) => {
      switch (args[0]) {
        case 'ls-remote':
          return '0123456789abcdef\trefs/heads/main';
        case 'remote':
