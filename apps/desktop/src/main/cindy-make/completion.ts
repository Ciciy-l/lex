...
/**
 * Run the completion sequence only through a verifier that returns the
 * canonical, currently registered Cindy Make worktree.  `commitCindyMakeChanges`
 * intentionally degrades individual Git errors into sparse display facts; a
 * verification failure must instead stay terminal so it cannot be mistaken for
 * a completed host-owned commit.
 */
export async function commitVerifiedCindyMakeChanges(
  verify: () => Promise<{ path: string }>,
  run: (args: string[], workingDir: string) => Promise<string>,
  message: string,
): Promise<Omit<CindyMakeCompletionMeta, 'reportedAt'>> {
  // Fail before the first Git invocation, even the read-only branch query.
  await verify();
  ...
  const git = async (args: string[]): Promise<string> => {
    ...
    // A regular Git failure is intentionally handled by commitCindyMakeChanges
    // as sparse completion facts. Only a failed authorization check is fatal.
    return run(args, verified.path);
  };
  const facts = await commitCindyMakeChanges(git, message);
  if (verificationFailure) {
    throw verificationFailure instanceof Error
      ? verificationFailure
      : new Error('Cindy Make worktree verification failed');
  }
  return facts;
}
