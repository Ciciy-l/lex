...
const TASK_AND_TEAMMATE_GUIDANCE = [
  '## 你可以开后台任务，也可以给伙伴发消息',
  `- \`start_session_task\` 会创建一条真正独立运行的 ${BRAND_NAME} 任务：它出现在用户的任务列表，有自己的工作过程、状态、停止、授权代答、结果和产物回传。用户明确说“开/建一个任务”“session 任务”“后台任务”时必须用它。它不会唤起任何伙伴。一次请求只启动一次。`,
  ...
  `- 收到 \`[Direct message from ${BRAND_NAME} Bot ...]\` 时，在自己的当前主任务里处理。确有答案、结果或澄清要回传时，用消息头里的 Bot id 作为 \`target_id\` 调用 \`send_to_agent\`；不要只为“收到”“好的”互相确认，也不要为了等回复自建循环。`,
...
function buildHomeGuidance(homeDir: string): string {
  return [
    '## 你有个自己的文件夹',
    `\`${homeDir}\` 是你的唯一 Home。身份、记忆、技能和默认工作区都归在这里，不继承当前项目或 ${BRAND_NAME} 的全局目录。`,
    ...
    '',
    `不要修改 Home 根部的宿主配置。可以按需发现 ${BRAND_NAME} 已安装的插件、Skill 和 MCP，并通过能力工具加入自己；已有连接沿用宿主授权。外部文件与命令遵循当前任务权限，workspace 是默认工作目录，不是只能访问这里。`,
  ].join('\n');
}
...
export function buildBotStableTier(input: BotSystemPromptInput): string {
  ...
  if (botModeEnabled && input.capabilities.routinesEnabled) {
    capabilityParts.push([
      ...
      `例行任务由 ${BRAND_NAME} 持久调度，不要改用 start_session_task、sleep 循环或系统定时器。按用户要求设置频率，不擅自增加一小时等结束限制。实际执行沿用当前伙伴权限，触发后的普通消息会回到这里。`,
    ].join('\n'));
  }
