...
vi.mock('../../maker-host/createDesktopProviderService.js', () => ({
  getDesktopProviderService: () => ({ listProviders: async () => [
    ...
    { id: 'omp-source', models: { omp: [{ id: 'omp-model' }] } },
  ] }),
}));
...
describe('owner-bound companion previews', () => {
  ...
  it('accepts an enabled OMP route for the same refinement and capability flow', async () => {
    const ompRoute = { agentKind: 'omp', providerId: 'omp-source', model: 'omp-model' };
    await generateBotCreationDraft({ prompt: 'Help organize a launch', modelRoute: ompRoute }, []);
    expect(h.catalog).toHaveBeenCalledWith(expect.objectContaining({ agentKind: 'omp' }));
    expect(h.generate.mock.lastCall![2]).toMatchObject(ompRoute);
  });
