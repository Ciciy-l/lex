...
describe('runtime harness selection', () => {
  ...
  it('stages OMP through the same deferred agent-switch transaction', async () => {
    const h = setup();
    const result = await setSessionRuntimeHarness(h.deps, {
      ...request,
      patch: { harness: 'omp', model: 'MiniMax-M2.5' },
    });
    ...
    expect(result).toMatchObject({
      ...
      pendingMutation: {
        source: 'agent',
        profile: { agentKind: 'omp', model: 'MiniMax-M2.5', providerId: 'openai' },
      },
    });
