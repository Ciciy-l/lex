...
vi.mock('../../maker-host/model-visibility-mirror.js', () => ({
  waitForModelVisibilityMirror: async () => {}, getModelVisibilityOverride: (_a: string, _p: string, model: string) => host.enabled && (model === 'luna' || model === 'omp-model' || (model === 'sol' && host.enableSecond)),
}));
...
describe('Bot control of the real Cindy default', () => {
  ...
  it('uses the same renderer-owned default route when the selected engine is OMP', async () => {
    const ompRoute = {
    ...
    const ompId = JSON.stringify(['omp', 'omp-source', 'omp-model']);
    host.agents = ['omp'];
    host.providers.mockResolvedValue([{
      id: 'omp-source', name: 'OMP source', source: 'user', connected: true, agents: ['omp'],
      auth: { method: 'apiKey' },
      routing: { omp: { upstream: 'https://example.invalid', authStrategy: 'api-key-header' } },
      models: { omp: [{ id: 'omp-model', status: 'active', mode: 'chat', defaultEnabled: true,
        efforts: ['low', 'medium'], defaultEffort: 'medium', supportsFastMode: false }] },
    }] as ProviderView[]);
    mirror(ompRoute);
    ...
    try {
      ...
      draft.patchVendorPrefs('omp', {
        model: ompRoute.model, providerId: ompRoute.providerId, effort: ompRoute.effort,
      });
      draft.switchVendor('omp');
      configureAppDefaultModelSelection(selection => {
        expect(draft.applyAppDefaultModelSelection(selection)).toBe(true);
        mirror(selection.route, selection.requestId);
      });

      expect(await changeAppDefaultModel(ompId, 'low')).toEqual({
        current: { ...ompRoute, effort: 'low' },
      });
      expect(draft.getDraft().lastByVendor.omp).toMatchObject({
        model: ompRoute.model, providerId: ompRoute.providerId, effort: 'low',
      });
    } finally {
      vi.unstubAllGlobals();
    }
