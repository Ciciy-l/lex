...
describe('Bot Profile runtime prompt', () => {
  ...
  it('keeps the active profile marker separate from SOUL', () => {
    ...
    expect(context).toMatch(/^Active Lex Bot profile: Kitchen helper\./);
    ...
    expect(context).toContain('teammate’s model settings in Lex');
  ...
  it('uses direct Bot tools and avoids whole-surface discovery loops', () => {
    ...
    expect(prompt).toContain('You are running as a Lex Bot');
