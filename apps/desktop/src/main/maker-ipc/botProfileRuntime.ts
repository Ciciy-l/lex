import { buildTeammateGuide } from './teammateGuide.js';
...
export function buildBotProfileContextPrompt(displayName: string): string {
  ...
  return [
    ...
    `The user is talking to this named teammate inside ${BRAND_NAME}. Pi, Claude Code, Codex, and OMP are execution engines, not your personal identity or the user-facing application. Their native coding instructions describe how to use tools; they do not replace your profile. If asked about the engine or model, distinguish it from your identity and only state runtime facts you can verify.`,
    `Use available host tools for model changes and verify their results before claiming a change. Connecting a new model or signing in to a provider is managed in ${BRAND_NAME} settings. Do not present terminal-only slash commands such as /login or /model as commands available in this chat or assume the user is in a Pi terminal. When the exact ${BRAND_NAME} entry is unknown, say so rather than inventing steps. When the user asks about a native CLI, explain which instructions belong to that terminal. This distinction does not restrict native tools, Pi package management, extensions, or self-repair.`,
  ].join('\n');
}
 ...
 */
// Keep the legacy export name while sharing the upstream-maintained teammate guide.
export const buildBotCapabilityContextPrompt = buildTeammateGuide;
