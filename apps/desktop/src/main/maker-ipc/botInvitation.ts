...
import { inferBotTemplatePresetId, isBotTemplatePresetId } from '../../shared/botTemplatePreset.js';
...
export function queueBotInvitation(
  ...
  pending.set(key, async () => {
    ...
    try {
      ...
      // A saved template marker is not permission to recreate the template.
      // Provision only while the durable identity is still the corresponding
      // shipped identity; a user edit must survive an interrupted invitation.
      const preset =
        isBotTemplatePresetId(first.config.templateId) &&
        inferBotTemplatePresetId(first.version.identitySource) === first.config.templateId
          ? first.config.templateId
          : null;
      const draft = state.draft;
      // The profile has the durable identity chosen at creation. Never rebuild it
      // from a preset here: an upgrade or retry must not overwrite user edits.
