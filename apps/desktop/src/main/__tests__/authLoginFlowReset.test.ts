...
describe('auth login-flow reset', () => {
  ...
  it('pins personal login to the explicitly selected realm and clears stale SSO discovery', () => {
    ...
    expect(actionPreamble).toContain(
      'const loginRealm = startsPersonalRealmFlow\n' +
        '    ? selectedPersonalAuthRealm\n' +
    ...
    expect(personalActionSetup).toContain(
      "if (startsPersonalRealmFlow && action.type !== 'request-code') pendingAuthRealm = loginRealm;",
    );
  });
  ...
  it('keeps cross-region email choices gated and preserves SSO after failed personal code sending', () => {
    ...
    expect(request.indexOf('pendingAuthRealm = selectedPersonalAuthRealm;')).toBeGreaterThan(
      request.indexOf('await client.requestCode('),
    );
  ...
  it('logs out only the current saved account and advances to the next restorable account', () => {
    ...
    expect(summariesBody).toMatch(
      /if \(!loggedOutKeys\.has\(key\)\) \{\s*byKey\.set\(key, \{ realm: resource\.realm, metadata: resource\.metadata \}\);/,
    );
    expect(summariesBody).toContain('if (!loggedOutKeys.has(key) && !byKey.has(key))');
    expect(summariesBody).toContain('{ realm: passport.realm, metadata: membership }');
    expect(summariesBody).toContain(
      'accountSummaryFromMetadata(key, entry.realm, entry.metadata, activeAccountKey)',
    );
