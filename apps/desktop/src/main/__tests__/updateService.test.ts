...
const appGetAppPath = vi.fn(() =>
  path.join(TEST_ROOT, 'Cindy.app', 'Contents', 'Resources', 'app.asar'),
);
...
const stageBundledWindowsUpdaterRuntime = vi.fn<() => 'staged' | 'fallback-safe' | 'blocked'>(
  () => 'staged',
);
...
// Relaunch now first quiesces durable Pi subagents. Keep this update-service
// suite focused on updater/marker semantics; the reclaim protocol is covered
// by its dedicated gate tests.
vi.mock('@cindy/maker-core/pi-subagent-runs', () => ({
  acquirePiSubagentLaunchFence: async () => async () => undefined,
  hasActivePiSubagentRunsSync: () => false,
  requestStopAllPiSubagentRunsSync: vi.fn(),
  stopAllPiSubagentRunsForExit: async () => true,
}));
...
describe.sequential('updateService', () => {
  describe('binary version checks after an applied update', () => {
    ...
    it('writes the target-version marker when the user applies the update', async () => {
    ...
    it('writes the target-version marker for an automatic update relaunch', async () => {
      const service = await freshUpdateService('darwin');
      const { consumeStartupBinaryUpdateMarker } = await import('../agent-binaries/startup-update');
      const exitSpy = vi.spyOn(process, 'exit').mockImplementation((() => undefined) as never);
      service.initUpdateService();
      try {
        ...
        const markerPath = path.join(TEST_USER_DATA, 'agent-binary-update-once.json');
        expect(fs.existsSync(markerPath)).toBe(false);
        await expect(ipcHandlers.get('update-relaunch-auto')?.({}, 'dark')).resolves.toMatchObject({
          accepted: true,
        });
        await vi.waitFor(() => {
          expect(spawnProcess).toHaveBeenCalledOnce();
    ...
    it('does not write the marker when Windows updater prerequisites block applying', async () => {
      ...
      try {
        ...
        expect(fs.existsSync(path.join(TEST_USER_DATA, 'agent-binary-update-once.json'))).toBe(
          false,
        );
      } finally {
        service.stopUpdateService();
    ...
    it('clears the marker when the Windows updater reports an asynchronous spawn error', async () => {
      ...
      try {
        ...
        ipcListeners.get('update-relaunch')?.({}, 'dark');
        await vi.waitFor(() => {
          expect(childListeners.has('error')).toBe(true);
        });
        const markerPath = path.join(TEST_USER_DATA, 'agent-binary-update-once.json');
    ...
    it('rejects beta channel setting writes on Linux arm64', async () => {
      ...
      try {
        ...
        await expect(setHandler?.({ sender: { id: 1 } }, { enableBeta: true })).rejects.toThrow(
          'This build does not support the beta update channel',
        );
  ...
  describe('checkForUpdate release asset URL resolution', () => {
    it('passes an absolute GitHub Release URL to the downloader unchanged', async () => {
      ...
      const releaseUrl =
        'https://github.com/Ciciy-l/lex/releases/download/v0.1.0/Lex-arm64-hotfix.zip';
  ...
  describe('startup update relaunch safety', () => {
    ...
    async function bootWithStagedPatch(
      options: {
        enabled?: boolean;
          ...
          expect(
            logInfo.mock.calls.some((call) =>
              String(call[0]).includes(
                'keeping newer staged patch after deferred channel-change clear',
              ),
            ),
          ).toBe(true);
        });
    ...
    it('aborts a manual relaunch when a channel change is still deferred', async () => {
      ...
      try {
        ...
        expect(logInfo.mock.calls.map((call) => String(call[0]))).toEqual(
          expect.arrayContaining([expect.stringContaining('executeRelaunch() aborted')]),
        );
        releaseProbe?.(false);
        await vi.waitFor(() => {
          expect(service.getUpdateStatus()).toBe('idle');
  ...
  describe('Windows updater prerequisites', () => {
    ...
    it.each([
      ...
      'keeps the patch and retry count when Runtime staging is $stageResult',
      async ({ stageResult, prerequisiteChecks }) => {
        readAutoUpdateSettings.mockReturnValue({ autoRelaunchOnIdle: false });
        checkWindowsUpdaterPrerequisites
        ...
        try {
          ...
          await vi.waitFor(() => {
            expect(stageBundledWindowsUpdaterRuntime).toHaveBeenCalledTimes(1);
