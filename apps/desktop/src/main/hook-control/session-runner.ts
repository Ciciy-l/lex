  ...
  const resolved = resolveHookSessionConfig(
    {
      readDefaults: () => {
        ...
        // 改写另一入口的新会话路由。
        const settings = readImDefaultSettings(sourceIm === 'slack' ? 'slack' : undefined);
        return {
          // OMP 尚未接入 hook 派发链路(IM 设置里也还没开放绑定):读到 'omp' 时
          // 按 Pi 回落 —— 两者在 IM_DEFAULT_SETTINGS 里是同档默认值,等 OMP
          // runtime 真正注册后再把 'omp' 一路放下去。
          agentKind: settings.agentKind === 'omp' ? 'pi' : settings.agentKind,
