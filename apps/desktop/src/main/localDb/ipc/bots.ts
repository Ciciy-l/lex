...
function botSessionAgentKind(config: { harness?: unknown }): 'cc' | 'codex' | 'pi' | 'omp' {
...
function defaultBotModelForConfig(config: Record<string, unknown>): string | null {
  const agentKind = botSessionAgentKind(config);
  if (agentKind === 'omp') return null;
  return agentKind === 'pi' ? NEW_BOT_DEFAULT_PI_MODEL : 'claude-sonnet-4-6';
}
  ...
  return {
    ...
    */
    ...
    skills: Array.isArray(config.skills)
      ...
      : [],
    capabilities: {
      // OMP has no product-owned fallback model. An old or incomplete OMP
      // profile must remain visibly unconfigured so the renderer resolves a
      // route from the connected provider catalog, rather than silently
      // presenting Claude's static default as an OMP model.
      model:
      ...
      harness:
        ...
        (config.harness === 'codex' || config.harness === 'pi' || config.harness === 'omp'
          ? config.harness
          : 'claude'),
      modelChain,
...
export function registerBotIpc(): void {
  ...
  const createBotCanonicalSessionUnlocked = async (
    ...
    const canonicalModel =
      primaryRoute.model.trim() ||
      (typeof config.model === 'string' ? config.model.trim() : defaultBotModelForConfig(config));
    if (!canonicalModel) {
      throwIpcError('PRECONDITION_FAILED', '请先连接模型供应商或选择伙伴模型');
    }
