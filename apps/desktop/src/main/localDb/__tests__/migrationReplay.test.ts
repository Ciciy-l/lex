  ...
  it.each(['missing table', 'legacy table', 'existing column'] as const)(
    ...
    (state) => {
      ...
      try {
        ...
        const migration = listMigrations(drizzleDir()).find((item) => item.seq === 105)!;
        ...
        for (let replay = 0; replay < 2; replay += 1) {
          const result = runMigrationReplay(db, { drizzleDir: stagedDir, currentVersion: 104 });
          expect(result.applied.map((item) => item.seq)).toEqual([105]);
          if (state === 'missing table') {
          ...
          expect(db.prepare("SELECT value FROM migration_meta WHERE key = 'schema_version'").pluck().get()).toBe('105');
        }
      } finally {
