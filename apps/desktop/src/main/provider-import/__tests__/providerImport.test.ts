...
describe('provider import URL parsing', () => {
  ...
  it('imports Google generateContent endpoints for every compatible engine', () => {
    const importId = createDraft(customPayload({
      endpoints: [{
  ...
  it('rejects an explicit OMP target for Google-native endpoints', () => {
    expect(
      createProviderImportDraftFromRest(
        importRest(
          customPayload({
  ...
  it('accepts generic authorization-code OAuth without importing tokens', () => {
    ...
    expect(previewProviderImport(importId, SCOPE, [])).toMatchObject({
      ...
      runtimes: [
        { agent: 'codex', modelCount: 0, willFetchModels: false },
        { agent: 'omp', modelCount: 0, willFetchModels: false },
      ],
    });
  });
