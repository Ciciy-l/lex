...
// OMP is routed through its managed loopback proxy. The proxy understands the
// three existing Claude/Codex-compatible wire shapes, but deliberately has no
// Google-native front door.
const OMP_PROTOCOLS: readonly ProviderWireProtocol[] = [
  'anthropic-messages',
  'openai-responses',
  'openai-chat',
];
...
function parseEndpoint(value: unknown, index: number): ParsedEndpoint {
  ...
  if (endpoint.targets !== undefined) {
    ...
    targets = endpoint.targets.map((target) => {
      ...
      if (target === 'omp' && !OMP_PROTOCOLS.includes(protocol)) {
        fail(`${label}.targets contains an unsupported OMP protocol`);
      }
...
function defaultTargets(protocol: ProviderWireProtocol): AgentKind[] {
  if (protocol === 'anthropic-messages' || protocol === 'google-generative-ai') {
    return protocol === 'google-generative-ai'
...
function parsePayload(value: unknown): ProviderImportDraft {
  ...
  for (const endpoint of endpoints) {
    // Generic OAuth credentials are held by Cindy's OAuth runner. OMP reaches it
    // through the managed proxy, while Pi's native runtime does not consume it.
    // Keep the compact vendor format useful by omitting Pi from implicit defaults;
    // an explicit Pi target remains an error so a vendor cannot accidentally
    // advertise an unusable runtime.
    const targets =
      endpoint.targets ??
