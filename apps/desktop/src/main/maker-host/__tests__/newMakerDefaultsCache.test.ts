...
describe('getRemoteNewMakerDefaults (device-link 远程草稿镜像)', () => {
  ...
  it('mirrors OMP preferences through the same four-engine snapshot', () => {
    seed({
      lastByVendor: {
        omp: {
          model: 'omp-model',
      ...
      modelChosenByVendor: { omp: true },
      fastModeByModel: { 'omp-model': true },
      effortByModel: {},
    });

    expect(getWorkerDefaultsFromNewMaker('omp')).toEqual({
      model: 'omp-model',
      effort: 'medium',
      fastMode: true,
      providerId: 'omp-source',
    });
    expect(getRemoteNewMakerDefaults('omp')).toMatchObject({
      model: 'omp-model',
      modelChosenByUser: true,
      permissionMode: 'auto',
      providerId: 'omp-source',
