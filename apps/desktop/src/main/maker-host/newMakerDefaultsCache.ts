 ...
 *
 * Vendor 名称差异: renderer 用 'cc' / 'codex' / 'pi' / 'omp'; worker spawn 路径用
 * 'claude-code' / 'codex' / 'pi' / 'omp'。getWorkerDefaultsFromNewMaker 内部做映射。
 */
type VendorKey = 'cc' | 'codex' | 'pi' | 'omp';
...
export function getWorkerDefaultsFromNewMaker(
  ...
  const vendor: VendorKey =
    workerAgent === 'claude-code' ? 'cc' : workerAgent === 'pi' ? 'pi' : workerAgent;
...
export function getRemoteNewMakerDefaults(
  ...
  const vendor: VendorKey =
    agentKind === 'claude-code' ? 'cc' : agentKind === 'pi' ? 'pi' : agentKind;
...
export function getNewMakerModelTuning(ownerScope: string, agent: 'claude-code' | 'codex' | 'pi' | 'omp',
