...
const mocks = vi.hoisted(() => ({
  ...
  runtimeManifest: null as { app: Record<string, never> } | null,
...
describe('startup binary version check policy', () => {
  it('reads the build-pinned runtime snapshot and reuses a verified older binary on ordinary startup', async () => {
  ...
  it.each([undefined, true])(
    'retains the existing upgrade flow with checkForUpdates=%s',
    async (checkForUpdates) => {
      installLocal('1.0.0');
      const provisioner = createBinaryProvisioner(config);
      await expect(provisioner.peekNeedsDownload({ checkForUpdates })).resolves.toBe(true);
      await expect(provisioner.prepare({ checkForUpdates })).resolves.toMatchObject({
        ready: true,
      });
  ...
  it('downloads and verifies the first installation even when upgrade checks are disabled', async () => {
    ...
    await expect(provisioner.prepare({ checkForUpdates: false })).resolves.toMatchObject({
      ready: true,
    });
    expect(mocks.download).toHaveBeenCalledOnce();
    expect(
      fs.existsSync(path.join(mocks.userDataDir, config.installSubdir, asset.version, '.verified')),
    ).toBe(true);
  });

  it.each(['unverified', 'missing'])(
    'repairs a %s local installation instead of falsely reporting ready',
    async (damage) => {
  ...
  it('preserves the highest actual self-updated runtime rather than selecting by directory name', async () => {
    ...
    await expect(provisioner.prepare({ checkForUpdates: false })).resolves.toEqual({
      ready: true,
  ...
  it('repairs a verified runtime whose executable no longer passes its local probe', async () => {
    ...
    const provisioner = createBinaryProvisioner({
      ...config,
      localVersionResolver: async () => null,
    });
    await expect(provisioner.peekNeedsDownload({ checkForUpdates: false })).resolves.toBe(true);
    await expect(provisioner.prepare({ checkForUpdates: false })).resolves.toMatchObject({
      ready: true,
  ...
  it('keeps the required-runtime offline fallback', async () => {
    ...
    await expect(
      createBinaryProvisioner(config).prepare({ checkForUpdates: false }),
  ...
  it('keeps optional runtimes unavailable when the manifest cannot be fetched', async () => {
    ...
    mocks.runtimeManifest = null;
    await expect(
      createBinaryProvisioner({ ...config, optionalAsset: true }).prepare({
  ...
  it('does not bypass manifest platform validation', async () => {
    installLocal('1.0.0');
    mocks.getVendorAsset.mockReturnValue({
      ...asset,
      file: 'claude-code/3.0.0/win32-x64/claude.exe',
    });
    await expect(
      createBinaryProvisioner(config).prepare({ checkForUpdates: false }),
