...
const mocks = vi.hoisted(() => ({
  userDataDir: '',
  isPackaged: true,
  runtimeManifest: null as Manifest | null,
  fetchManifest: vi.fn(),
  download: vi.fn(),
  execFile: vi.fn(),
...
vi.mock('../../manifestService.js', () => ({
  ...
  resolveManifestAssetUrl: (base: string, file: string) => `${base}/${file}`,
}));
vi.mock('../runtime-manifest.js', () => ({
  getRuntimeManifest: () => mocks.runtimeManifest,
  getRuntimeAssetBaseUrl: () => 'https://runtime-cdn.test',
}));
  ...
  mocks.execFile
    ...
    .mockImplementation(
      (
        ...
        if (command === '/bin/sh') {
          const commandName = [...mocks.systemCommands.keys()].find((name) =>
            args.join(' ').includes(name),
          );
          callback(null, commandName ? mocks.systemCommands.get(commandName)! : '', '');
          return;
        }
        try {
          callback(null, fs.readFileSync(command, 'utf8'), '');
        } catch {
          callback(new Error('executable missing'), '', '');
        }
      },
    );
  mocks.download
    .mockReset()
    .mockImplementation(async ({ targetPath, url }: { targetPath: string; url: string }) => {
...
describe('shared startup policy through the real binary preparation chain', () => {
  it.each(['darwin', 'win32', 'linux'] as const)(
    ...
    async (platform) => {
      ...
      for (const kind of ['claude-code', 'codex', 'pi'] as const) {
        ...
        await expect(binaries.prepare(kind, quiet)).resolves.toMatchObject({
          ready: true,
          path: binaryPath,
        });
      }
  ...
  it('consumes a matching marker once and retains the upgrade decision for retries and other vendors', async () => {
    ...
    expect(fs.existsSync(path.join(mocks.userDataDir, 'agent-binary-update-once.json'))).toBe(
      false,
    );
    ...
    await expect(binaries.prepare('pi', quiet)).resolves.toMatchObject({
      ready: true,
      path: path.join(mocks.userDataDir, 'pi', '2.0.0', 'pi'),
    });
  ...
  it.each([false, true])(
    ...
    async (allowUpdates) => {
      ...
      try {
        ...
        expect(binaries.getReadyBinaryPath('pi')).toBe(
          allowUpdates ? path.join(mocks.userDataDir, 'pi', '2.0.0', 'pi') : localPath,
        );
  ...
  it('allows an explicit update check without changing the default startup decision', async () => {
    ...
    installManaged('codex');
    await expect(
      binaries.prepare('pi', { ...quiet, checkForUpdates: true }),
  ...
  it('does not resurrect a withdrawn optional runtime through the shared policy', async () => {
    installManaged('pi');
    mocks.runtimeManifest = { app: {} } as Manifest;
    await expect(binaries.prepare('pi', quiet)).resolves.toMatchObject({
      ready: false,
      error: 'asset_missing',
    });
...
describe('Linux local discovery before any update download', () => {
  it('keeps the previously used system Codex when a newer CDN release appears on the next startup', async () => {
    ...
    const systemPath = writeExecutable(
      path.join(tempRoot, 'system-codex'),
      'codex-cli ' + PINNED_CODEX_VERSION,
    );
    mocks.systemCommands.set('codex', systemPath);
    mocks.runtimeManifest = { app: {} } as Manifest;
    await expect(
      binaries.prepare('codex', { ...quiet, checkForUpdates: true }),
    ).resolves.toMatchObject({ ready: true, path: systemPath });
    expect(mocks.download).not.toHaveBeenCalled();
    await reloadBinaries('linux');
    mocks.runtimeManifest = releaseManifest(newerStableVersion(PINNED_CODEX_VERSION));
    await expect(binaries.peekNeedsDownload('codex')).resolves.toBe(false);
    await expect(binaries.prepare('codex', quiet)).resolves.toMatchObject({
      ready: true,
      path: systemPath,
    });
    expect(mocks.download).not.toHaveBeenCalled();
  ...
  it.each([false, true])(
    'only reuses a private cached runtime when it is executable (broken=%s)',
    async (broken) => {
      await reloadBinaries('linux');
      const runtimeRoot = path.join(mocks.userDataDir, 'agent-runtime', 'codex');
      const binaryPath = writeExecutable(
        path.join(runtimeRoot, 'bin', 'codex'),
        broken ? 'invalid output' : 'codex-cli ' + PINNED_CODEX_VERSION,
      );
