...
describe('git-review IPC payload guards', () => {
  it('accepts only fixed commit IDs for lightweight commit file lists', () => {
    expect(parseCommitFilesPayload({ sessionId: 's1', oid: HEX_OID })).toEqual({
      sessionId: 's1',
      oid: HEX_OID,
    });
    for (const oid of ['HEAD', SHORT_HEX_OID, `${HEX_OID}^`, '../commit']) {
      expect(() => parseCommitFilesPayload({ sessionId: 's1', oid })).toThrow(/\[INVALID_PARAMS\]/);
    }
  });

  it('rejects unsafe file targets before write operations reach stageOps', () => {
    expect(() => parseTarget({ source: 'unstaged', path: '../secret.txt' })).toThrow(
      /\[INVALID_PARAMS\]/,
    );
    expect(() => parseTarget({ source: 'unstaged', path: 'C:\\Users\\secret.txt' })).toThrow(
      /\[INVALID_PARAMS\]/,
    );
    expect(() =>
      parseTarget({ source: 'unstaged', path: 'file.txt', oldPath: '../old.txt' }),
    ).toThrow(/\[INVALID_PARAMS\]/);
  });

  it('rejects unsafe open-file paths before resolving the worktree file', () => {
    expect(() => parseOpenFilePayload({ sessionId: 's1', path: '../secret.txt' })).toThrow(
      /\[INVALID_PARAMS\]/,
    );
    expect(() => parseOpenFilePayload({ sessionId: 's1', path: 'C:\\Users\\secret.txt' })).toThrow(
      /\[INVALID_PARAMS\]/,
    );
  });
  ...
  it('rejects non-hex commit ids for commit diffs and preview requests', () => {
    expect(() => parseCommitDiffPayload({ sessionId: 's1', oid: 'HEAD' })).toThrow(
      /\[INVALID_PARAMS\]/,
    );
    expect(() =>
      parseImagePreviewPayload({
...
describe('git-review write busy gate', () => {
  ...
  it('rejects opening a controlled-side file with the local shell', async () => {
    ...
    await expect(
      openReviewFile('s1', 'file.txt', { resolveScope }, openPath),
    ).rejects.toMatchObject({ code: 'PRECONDITION_FAILED' });
