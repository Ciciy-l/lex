...
export interface MakerScheduleRunnerDeps {
  ...
  /**
   * Headless 默认路由的实时快照。没有静态默认模型的 agent 用它成对解析
   * model/providerId；
...
/** 原生 provider 路由热切失败；继续派发会把任务发给旧 provider，必须在 vendor 前站下。 */
class QueuedNativeRouteSyncError extends Error {}
...
export class MakerScheduleRunner implements ScheduleRunner {
  ...
  private async fireInner(
    ...
    if (!model) {
      throw new Error(
        `schedule route unavailable: no connected model source for ${effectiveAgentKind}`,
      );
    }
    if (
      ...
      onAccepted: async (queuedPermissions) => {
        ...
        try {
          ...
          if (
            ...
            err instanceof QueuedNativeRouteSyncError ||
  ...
  private async applyQueuedHeartbeatRouting(
    ...
    // 与直发路径一致：Pi / OMP 的 provider 是原生进程态。schedule 显式 model/source
    // 或停用轴改道时，需要把 provider-model 一起重申；只写 provider store 对
    // Pi BYOM 无效，即使 model 字符串没变也不能跳过。
    const mustSyncNativeProviderRoute =
      (live.agentKind === 'pi' || live.agentKind === 'omp')
      && (explicitModel !== undefined || applyProviderId !== null);
    let modelApplied = true;
    if (modelChanged || mustSyncNativeProviderRoute) {
      try {
        if (mustSyncNativeProviderRoute) {
        ...
        if (mustSyncNativeProviderRoute) {
          throw new QueuedNativeRouteSyncError(
            `schedule native route sync failed before queued dispatch (model "${targetModel}", provider "${nextProviderId ?? 'cindy'}"): ${err instanceof Error ? err.message : String(err)}`,
            { cause: err },
          );
        }
