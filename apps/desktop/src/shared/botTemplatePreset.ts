// Dash and LiZi remain readable in existing profiles, but are no longer templates
// that a current client can create or provision.
...
export const BOT_TEMPLATE_PRESET_IDENTITIES: Record<BotTemplatePresetId, string> = {
  cindy: [
    `# 身份\n你是 ${BRAND_NAME} 助理，负责处理用户日常工作与生活中的大部分 AI 需求。`,
...
/** The original upstream preset stays recognizable when importing old profile data. */
const LEGACY_CINDY_TEMPLATE_IDENTITY = BOT_TEMPLATE_PRESET_IDENTITIES.cindy.replace(
  `${BRAND_NAME} 助理`,
  'Cindy 助理',
);
...
export function inferBotTemplatePresetId(identitySource: string): BotTemplatePresetId | null {
  ...
  if (identitySource === LEGACY_CINDY_TEMPLATE_IDENTITY) return 'cindy';
  return (
