...
function verifyPackagedSourceMetadata({ appName, platform, arch, expectedCommit }) {
  ...
  const metadataPath =
    platform === 'darwin'
      ? path.join(packagedDir, `${appName}.app`, 'Contents', 'Resources', 'lex-source.json')
      : path.join(packagedDir, 'resources', 'lex-source.json');
  if (!fs.existsSync(metadataPath)) {
    throw new Error(`packaged Lex source metadata missing at ${metadataPath}`);
  }
  let metadata;
  try {
    metadata = JSON.parse(fs.readFileSync(metadataPath, 'utf8'));
  } catch (err) {
    throw new Error(`packaged Lex source metadata is invalid JSON: ${err.message}`);
  }
  if (
    !metadata ||
    ...
    throw new Error(`packaged Lex source metadata has invalid sourceCommit/builtAt: ${metadataPath}`);
  }
  if (expectedCommit && metadata.sourceCommit !== expectedCommit) {
    throw new Error(
      `packaged Lex source metadata commit mismatch: expected ${expectedCommit}, got ${metadata.sourceCommit}`,
    );
  }
