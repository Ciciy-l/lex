...
- You then choose where to install: global, a known project, or another directory. Lex extracts metadata from the file automatically and lists the skill with that name and description.
...
- Renaming a local skill while publishing keeps its enabled or disabled setting. If renaming fails, Lex restores the original folder and content.
- The switch is local to this device and Lex profile. It does not edit external CLI settings, sync to other devices, or override a native engine's own disabled state. Enable it again to let the engine discover it normally.
...
- External source imports remove only their discovery links and keep the external source files. Package-owned or built-in skills must be removed through their owning package; their uninstall action is unavailable here. Skills provided by Lex plugins are managed on the owning plugin's page, including when discovered through shared skill links.
...
- Uninstalling a shared copy affects external CLIs that use it. It does not unpublish a skill or delete other users' copies. If file removal fails, Lex keeps the installation. If cleanup is interrupted, the notice offers **Retry cleanup**. The unfinished operation survives closing, reloading, and restarting Lex; reopen Skills in the same Lex profile and account to continue. Lex pauses conflicting installations until cleanup completes. Retrying never moves files to the trash a second time and preserves externally restored or replaced content.
