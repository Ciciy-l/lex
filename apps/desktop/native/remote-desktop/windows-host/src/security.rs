...
/** Only the packaged Lex Main executable may connect to the privileged broker. */
fn is_lex_main_executable(name: &std::ffi::OsStr) -> bool {
    let name = name.to_string_lossy();
    name.eq_ignore_ascii_case("Lex.exe") || name.eq_ignore_ascii_case("LexDev.exe")
}
...
pub fn authorize_client(pid: u32) -> Result<(Handle, u32)> {
    ...
    if !client_image.parent().is_some_and(|p| same_file(p, install))
        || !client_image.file_name().is_some_and(is_lex_main_executable)
    {
        return denied();
    }
    Ok((client, session))
...
#[cfg(test)]
mod tests {
    use super::is_lex_main_executable;
    use std::ffi::OsStr;

    #[test]
    fn confines_the_broker_to_lex_main_executables() {
        assert!(is_lex_main_executable(OsStr::new("Lex.exe")));
        assert!(is_lex_main_executable(OsStr::new("lexdev.exe")));
        assert!(!is_lex_main_executable(OsStr::new("Cindy.exe")));
        assert!(!is_lex_main_executable(OsStr::new("explorer.exe")));
    }
}
