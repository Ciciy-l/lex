...
function toCatalogModel(
  ...
  const model: CatalogModel = {
    ...
    // 保留明确的 text-only 声明；Pi 传输仅对真正缺失能力的模型采用未知默认。
    ...(m.supportsImageInput !== undefined ? { supportsImageInput: m.supportsImageInput } : {}),
...
function defaultWireProtocol(agent: AgentKind): ProviderWireProtocol {
  ...
  if (agent === "claude-code" || agent === "omp") return "anthropic-messages";
