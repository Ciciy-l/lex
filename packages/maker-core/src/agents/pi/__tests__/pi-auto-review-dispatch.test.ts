...
describe('pi auto-review dispatch & spawn config (mocked pi process)', () => {
  ...
  it.each(['ask', 'auto', 'bypassPermissions'] as const)('installs an exact user command without additional confirmation in %s', async (permissionMode) => {
    ...
    try {
      ...
      expect(prompt?.message).toContain(`[${BRAND_NAME} internal Pi extension operation receipt]`);
      ...
      expect(prompt?.message).toContain('The resulting package state is available after starting a new Pi task.');
      expect(prompt?.message).toContain('Lex requested active local Pi tasks including this task to stop');
      expect(prompt?.message).toContain('restart Lex to finish refreshing Pi packages');
  ...
  it.each([true, undefined])('explains missing install projection using native success evidence %s', async (nativeCommandSucceeded) => {
    ...
    try {
      ...
      if (nativeCommandSucceeded) {
        ...
        expect(prompt).toContain(`${BRAND_NAME} cannot currently confirm the enabled state`);
        expect(prompt).not.toContain(`${BRAND_NAME} could not leave the extension installed and enabled`);
      } else {
        expect(prompt).not.toContain('The native Pi installation succeeded.');
        expect(prompt).toContain(`${BRAND_NAME} could not leave the extension installed and enabled`);
      }
    } finally {
      await handle.close();
    }
  ...
  it('keeps direct native success while publishing bounded recovery on host failure', async () => {
    ...
    try {
      ...
      expect(prompt).toContain('restart Lex to finish refreshing Pi packages');
      await vi.waitFor(() => expect(events.some((event) => (
        event.type === 'text'
        && String((event.data as { text?: string }).text).includes('"runtimeConvergence":"partial"')
      ))).toBe(true));
