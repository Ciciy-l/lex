...
function supportsDirectorySymlinkFixture(): boolean {
  const probe = mkdtempSync(path.join(tmpdir(), 'pi-project-cli-link-probe-'));
  try {
    const target = path.join(probe, 'target');
    mkdirSync(target);
    symlinkSync(target, path.join(probe, 'link'), process.platform === 'win32' ? 'junction' : 'dir');
    return true;
  } catch (error) {
    const code = (error as NodeJS.ErrnoException).code;
    // A Windows account without reparse-point permission cannot exercise the
    // real escape branch. Keep the production check intact and let CI / capable
    // machines cover it; unexpected setup failures still surface.
...
const symlinkFixtureIt = supportsDirectorySymlinkFixture() ? it : it.skip;
...
describe('collectPiProjectResourceCliPaths', () => {
  ...
  symlinkFixtureIt('skips escaped symlinks and settings files', () => {
    const repo = makeRepo();
