...
function piManagedPackageReceiptPrompt(
  ...
  const build = (value: Record<string, unknown>): string =>
    [
      ...
      `Reply in the user language. If cancelled is true, say only that the operation was cancelled. For any successful operation, state the result and name/version when present, then say that ${BRAND_NAME} requested active local Pi tasks including this task to stop; do not claim every task has already stopped. The resulting package state is available after starting a new Pi task. If runtimeConvergence is partial or this task remains active, tell the user to restart ${BRAND_NAME} to finish refreshing Pi packages. Do not enumerate non-blocking compatibility notices and do not direct the user to Settings. Mention compatibility details only when they blocked the requested result. If outputTruncated is true, say that ${BRAND_NAME} omitted unusually large technical details.`,
    ].join('\n');
  ...
  return [
    ...
    `${BRAND_NAME} already handled this exact command through its managed Pi extension store. Do not run bash, the Pi CLI, or cindy_pi_extension again.`,
    `Reply in the user language. Say whether the operation succeeded and that ${BRAND_NAME} omitted unusually large compatibility details. If it succeeded, say that ${BRAND_NAME} requested active local Pi tasks including this task to stop without claiming every task has stopped, and tell the user to start a new Pi task. If runtimeConvergence is partial or this task remains active, tell the user to restart ${BRAND_NAME} to finish refreshing Pi packages.`,
  ].join('\n');
}
...
export class PiAgent extends BaseAgent {
  ...
  private async buildSettingsJsonPreservingUserKeys(
    ...
    if (opts.fileOps) {
      return opts.shellPath
        ? JSON.stringify({ ...JSON.parse(built), shellPath: opts.shellPath }, null, 2) + '\n'
        : built;
    }
  ...
  private async startSessionWhileRunning(opts: StartSessionOptions): Promise<AgentSessionHandle> {
    ...
    try {
      ...
      // The bridge creates its own Bash tool: use the final settings shell,
      // including the user's stable override, rather than just auto-detection.
      // Remote launches never read or transmit controller-local shell paths.
      const runtimeShellPath = remote
        ? undefined
        : collectPiUserSettingsPassthrough(
            await fs.readFile(joinRemotePosixPath(configHome, 'settings.json'), 'utf8'),
          ).shellPath;
