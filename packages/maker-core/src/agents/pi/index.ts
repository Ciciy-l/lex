...
export class PiAgent extends BaseAgent {
  ...
  private async buildSettingsJsonPreservingUserKeys(
    ...
    if (!opts.shellPath || collectPiUserSettingsPassthrough(merged).shellPath) return merged;
    return JSON.stringify({ ...JSON.parse(merged), shellPath: opts.shellPath }, null, 2) + '\n';
  }
