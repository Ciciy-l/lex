...
/**
 * Codex ships as a directory distribution so it can keep its companion
 * binaries alongside the app-server. Do not accidentally exercise the legacy
 * single-binary cache when a developer still has one from a prior pin.
 */
const binary = (name: 'claude-code' | 'codex') => name === 'codex'
  ? path.join(repo, 'apps', 'codex-package-bin', `${process.platform}-${process.arch}`, 'bin', `codex${suffix}`)
  : path.join(repo, 'apps', 'claude-code-bin', `${process.platform}-${process.arch}`, `claude${suffix}`);
const roots: string[] = [];
// This test performs two real app-server launches. Each RPC keeps a strict 15s
// fail-closed deadline, so the test needs room for both launches and for their
// process handles to close before its disposable fixture is removed.
const nativeSkillDiscoveryTestTimeout = 40_000;
afterEach(() => {
  for (const root of roots.splice(0)) {
    // Windows can briefly retain a closed child process' working directory.
    // Retry only teardown's known transient filesystem errors; a persistent
...
function rpc(args: { binary: string; args: string[]; cwd: string; env: NodeJS.ProcessEnv; initial: unknown;
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      failure = new Error(`Native Skill discovery timed out${stderr ? `: ${stderr}` : ''}`);
      child.kill('SIGKILL');
    }, 15_000);
...
describe('pinned native Skill activation', () => {
  ...
  it.skipIf(!fs.existsSync(binary('codex')))('Codex matches physical Skill identity through a shared discovery link', async () => {
    ...
    writeSkill(source, 'native-name');
    // Project discovery is native, isolated from a developer's real Windows
    // profile, and still exercises the physical source ↔ junction identity.
    const alias = path.join(cwd, '.agents', 'skills', 'alias');
  ...
  }, nativeSkillDiscoveryTestTimeout);
});
