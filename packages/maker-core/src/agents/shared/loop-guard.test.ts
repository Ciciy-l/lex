...
describe('ToolLoopGuard', () => {
  ...
  it('does not treat changing reads, intervening writes, or new inputs as stable rotation', () => {
    for (const mode of ['changing-output', 'write', 'new-input'] as const) {
      ...
      for (let i = 0; i < 512; i++) {
        ...
        expect(feed(guard, String(i), 'grep', input, output).kind).toBe('ok');
      }
    }
  });

  it('detects stale search cycles despite occasional output reordering', () => {
    const guard = new ToolLoopGuard();
  ...
  it('resets long read evidence on a new turn', () => {
    ...
    for (let turn = 0; turn < 3; turn++) {
      guard.resetTurn();
      for (let i = 0; i < 100; i++) {
        expect(feed(guard, String(turn) + '-' + i, 'find', { pattern: 'file-' + i % 20 }, 'file-' + i % 20).kind).toBe('ok');
      }
    }
  });
  ...
  it.each([
    ['powershell', 'Get-Content -Tail 8 C:\\logs\\gate.log'],
    ['powershell', 'Get-Content -LiteralPath \'C:\\Build Logs\\gate.log\' -Tail 8'],
    ['exec', 'Get-Content -Tail 8 -Path C:\\logs\\gate.log; Get-Content C:\\logs\\other.log -Tail 5'],
    ['exec', 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe -NoProfile -Command "Get-Content -Tail 8 C:\\logs\\gate.log"'],
    ['exec', '"C:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command "get-content -literalpath C:\\logs\\gate.log -tail 8"'],
  ...
  it.each([
    'Get-Content C:\\src\\source.ts -Tail 8',
    'Get-Content C:\\logs\\gate.log',
    'Get-Content C:\\logs\\gate.log -Tail 8; npm test',
    'Get-Content C:\\logs\\gate.log -Tail 8 > C:\\logs\\out.log',
    'Get-Content "$env:TEMP\\gate.log" -Tail 8',
    'Get-Content C:\\logs\\gate.log -Tail 8 | Select-String failed',
  ...
  it('keeps failing PowerShell log reads in the repeat detector', () => {
    ...
    let verdict: ToolLoopGuardVerdict = { kind: 'ok' };
    for (let i = 0; i < 4; i++) {
      verdict = feed(guard, String(i), 'powershell',
        { command: 'Get-Content C:\\logs\\gate.log -Tail 8' }, 'file not found', true);
    }
    expect(verdict.kind).toBe('hard');
  });
  ...
  ])('does not exempt ambiguous or mixed shell commands: %s', (command) => {
    const guard = new ToolLoopGuard();
    let verdict: ToolLoopGuardVerdict = { kind: 'ok' };
  ...
  it('does not let successful log polls erase ordinary repeated calls', () => {
    const guard = new ToolLoopGuard();
    for (let i = 0; i < 4; i++) {
      const ordinary = feed(guard, 'read-' + i, 'read', { path: 'source.ts' }, 'same source');
      expect(ordinary.kind).toBe(i === 3 ? 'hard' : 'ok');
      expect(feed(guard, 'poll-' + i, 'exec', { command: 'tail -8 /tmp/gate.log' }, 'waiting').kind).toBe('ok');
    }
  });
