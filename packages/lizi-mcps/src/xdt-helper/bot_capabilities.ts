...
export interface AppDefaultModelRoute {
  harness: 'claude' | 'codex' | 'pi' | 'omp'; providerId: string | null;
...
const FIND_BOT_CAPABILITIES_CORE =
  `按需查找 ${BRAND_NAME} 已有的 Skill、MCP 连接或内置工具集，返回可用性和当前伙伴是否已加入。`;
const FIND_BOT_CAPABILITIES_PLUGIN_SENTENCE =
  `插件用 ${BRAND_NAME} 提供的 cindy 网关的 ghost_list / ghost_info 发现并直接按现有授权调用。`;
...
export function registerBotCapabilityTools(
  ...
  registry.register({
    ...
    category: "bots",
    description:
      `把查到的已有能力加入当前伙伴，或从当前伙伴移除。复用 ${BRAND_NAME} 已有安装和连接，不修改共享源、凭证或全局开关。新挂载在下一轮生效；当前轮不要声称已有尚未挂载的工具。`,
