...
const DESCRIPTION = [
  ...
  '- agent: worker agent 类型 (codex / claude-code / pi / omp)',
  ...
  '- fast: 可选 boolean, 是否给 worker 开启 Fast 模式 (更快输出)。用户明确说「fast / 快速 / 开/关 fast」时显式传。对 codex 与 pi worker 生效 (且需所选模型声明 supportsFastMode; 否则自动降级为关); claude-code 与 omp worker 忽略此参数 (其 agent 层没有 Fast 模式)。不传则继承默认 (New Maker 面板默认或 lead session 的 fastMode)。',
