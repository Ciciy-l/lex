...
export function registerCreateTeammateTool(
  ...
  registry.register({
    ...
    category: 'bots',
    description: `Create a new ${BRAND_NAME} Bot teammate directly when the user asks for one. Do not write a template file or tell the user to create it manually. Only a name is required. Follow the user’s ${BRAND_NAME} default model chain and shared Bot baseline. Leave unspecified identity details open; the new teammate speaks for itself using its own runtime and memory. The returned bot id is immediately usable as send_to_agent target_id; creation alone does not start hidden work.`,
    inputShape: {
    ...
    handler: async ({ name, description, identity_source }: {
      ...
      if (!callerSessionId) {
        return errorPayload('NOT_A_BOT_SESSION', `当前调用未绑定 ${BRAND_NAME} 伙伴任务。`);
      }
