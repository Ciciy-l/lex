...
export function registerCreateTeammateTool(
  ...
  registry.register({
    ...
    category: 'bots',
    description: `Create a new ${BRAND_NAME} Bot teammate directly when the user asks for one. Do not write a template file or tell the user to create it manually. Use the official default model and an empty capability grant; the user can customize the profile later. Include a short natural first greeting in the user's language. The returned bot id is immediately usable as send_to_agent target_id; creation alone does not start hidden work.`,
    inputShape: {
