...
describe("cindy_helper MCP server", () => {
  ...
  it("exposes one Session-task lifecycle without a named-Bot task path", async () => {
    ...
    try {
      ...
      expect(sessionTaskTool?.description).toContain(`real independent ${BRAND_NAME} Session task`);
      expect(sessionTaskTool?.description).toContain(`never calls a ${BRAND_NAME} Bot`);
      expect(
