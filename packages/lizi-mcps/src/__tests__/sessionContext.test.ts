...
describe('dynamic lizi MCP session context', () => {
  ...
  it('passes host-owned Bot identity to session_search without exposing it as tool input', async () => {
    ...
    const listed = parse(await tools(cfg.instance).list_tools.handler({ category: 'search' }) as never);
    const sessionSearch = (listed.tools as Array<{ name: string; description: string }>)
      .find((tool) => tool.name === 'session_search');
    expect(sessionSearch?.description).toContain(`${BRAND_NAME} Bot`);
    expect(sessionSearch?.description).not.toContain('Cindy Bot');
