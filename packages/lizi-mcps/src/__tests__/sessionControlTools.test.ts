...
describe('cindy_helper session control tools', () => {
  ...
  it('forwards a complete OMP selection through the ordinary runtime switch contract', async () => {
    const { deps, registry } = setup();
    expect(parse(await registry.call('set_session_runtime', {
      session_id: 'target', harness: 'omp', model: 'omp-model', provider_id: 'omp-source', expected_generation: 1,
    }))).toMatchObject({ ok: true });
    expect(deps.setSessionRuntime).toHaveBeenCalledWith({
      targetSessionId: 'target',
      expectedGeneration: 1,
      patch: { harness: 'omp', model: 'omp-model', providerId: 'omp-source' },
    });
  });
