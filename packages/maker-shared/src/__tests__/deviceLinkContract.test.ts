...
describe('device-link shared contract', () => {
  ...
  it('maps agent not-authenticated errors to guidance copy', () => {
    ...
    expect(describeAgentAuthError('claude-code not authenticated: no_key'))
      .toContain('设置 → 模型供应商');
    expect(describeAgentAuthError('claude-code not authenticated: no_key')).toContain('Lex');
    ...
    expect(describeAgentAuthError('omp not authenticated: no_key')).toContain('OMP');
